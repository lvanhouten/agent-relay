...
- **03-read-screen-mcp-tool:** added a generic-failure branch for a malformed/unexpected board reply (`ok:false` with no `ended` field, or a falsy reply) — not spelled out in the brief but consistent with "an RPC-level failure maps to a generic read-failure error"; avoids mis-mapping a malformed reply to one of the two named failures. **Contract:** no VC-n affected.
