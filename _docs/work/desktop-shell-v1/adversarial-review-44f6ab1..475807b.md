...
|----|----------|------|---------|--------|
| W1 | WARNING | 75 | Tombstone status decode hand-rolled in 3 places, wording already drifting | (open) |
| W2 | WARNING | 62 | Alt+1..9 document listener has no focus/target guard — hijacks keys in text fields | (open) |
| W3 | WARNING | 58 | Notification toggle branches on stale `enabled` — first re-enable click silently no-ops | (open) |
| W4 | WARNING | 55 | `resolveSelection` ref-fallback is subtle, inline, and untested — against the diff's own precedent | (open) |
| N1 | NOTE | 65 | Comments cite pruned `brief 0N` artifacts; one is stale on arrival | (open) |
| N2 | NOTE | 45 | Selected tombstone becomes a frozen ghost if the board's 20-cap ring evicts it | (open) |
| N3 | NOTE | 35 | Unbounded/unsanitized session name flows into the OS Notification title/body | (open) |
| N4 | NOTE | 32 | Alt+N listener re-subscribes every render (unmemoized `liveSessions`) | (open) |
