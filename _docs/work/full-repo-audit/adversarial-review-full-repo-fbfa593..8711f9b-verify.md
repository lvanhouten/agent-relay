...
| N2(new, r1) | ✅ Resolved `eca67d4` | ❌ **Regressed** | This *is* the TTL mechanism driving C1's regression above — the fix for N2-new's latency concern (`BOOT_TTL_MS` short-circuit) is exactly what reopens C1 sub-defect 3. Same root cause, same evidence. |
...
**Tally:** 12 of 14 round-2 items Confirmed closed · **2 Regressed (C1, N2-new — same root cause)**.
...
|----|----------|------|---------|--------|
| C1 | CRITICAL | 80 | `refreshBoot`'s TTL short-circuit reopens C1's re-corruption window | (open) |
| W1 | WARNING | 90 | `switchboard_end_line` skips `forgetLine` on RPC rejection | (open) |
