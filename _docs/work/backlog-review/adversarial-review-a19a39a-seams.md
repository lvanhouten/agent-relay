...
**Remediation (2026-07-07, branch `fix/backlog-review-remediation`):** all five fix-first findings fixed, one commit each, in ranked order — resolutions annotated in each slice doc's finding + status column. Remaining warnings/notes (static W2/N1, attention W1, notify W2/W3/W4 + notes, rdp W2 + notes, qol W2–W4 + notes, seams S1–S3) are still open.
