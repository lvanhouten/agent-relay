...
**Resolution (fixed):** both halves extracted — `core/attention.ts` (attention W1) and `core/searchReadout.ts` (qol N3), each tested. The third-occurrence-means-checklist tripwire stands.
...
**Remediation (2026-07-07, branch `fix/backlog-review-remediation`):** **every warning in the range is fixed**, one commit each — first the five fix-first findings in ranked order, then the second pass (notify W2/W3/W4, qol W2/W3/W4, static W2, rdp W2, attention W1). A closing **notes sweep** then fixed everything actionable: static N1/N2, attention N1/N2, notify N1 (id-reuse edge)/N4/N5, rdp N1/N3/N5 (N5 turned out deeper than flagged — `-File` never splits comma-joined arrays, so name forwarding had never worked), qol N1–N5, and both halves of seams S1. Resolutions are annotated per-finding in each slice doc. Remaining open, all deliberate: notify N2 (dedup could suppress legit re-alerts), rdp N2 (refactor cost > payoff at two scripts), rdp N4 (deferred to the first real phone test), qol N6 (cosmetic)/N7 (backlogged to scoped-tokens), notify N3 + rdp N6 + attention N3 (accepted), S2 (a practice), S3 (sanctioned-deferred to templates phase 2).
