...
- Enforcement points: `src/ws.js` tags each connection with its scope at upgrade and filters inbound frames (a `read` connection's `input` frames are dropped, not an error); `src/api.js` gates mutating routes on `full`. Both already funnel through single gates (`checkToken`, `authMiddleware`), so scope threading stays local. (medium) — Note 2026-07-07: desktop shell slice 2 (`2026-07-07-desktop-spectator-panes.md`) builds this exact WS frame filter for spectator attaches (`?mode=spectator`, no-input/no-resize); this feature reuses that filter with a token-derived scope instead of a query param. One design, two consumers.
