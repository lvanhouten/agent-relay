...
- Chat view vs. terminal truth: the transcript lags the PTY (and omits raw TUI state like permission prompts mid-render); the view must not pretend to be live when it's behind.
...
- A concrete consumer for message content: the mobile chat view being actively missed, or notification bodies needing the blocked command.
