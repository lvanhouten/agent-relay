...
- **Pane grid in `DesktopShell`** — 2+ terminals side by side via **`react-resizable-panels`** (decided 2026-07-15). Unfocused panes attach as spectators; the focused pane reattaches interactive and owns sizing. (medium-large)

  **Layout-library decision (2026-07-15): `react-resizable-panels`, not `dockview`.** The interaction model is a *focus grid*, not an IDE workspace — ADR-0005 already fixed it (focused pane interactive, rest spectators, focus-change = detach+reattach), so dockview's headline features (draggable tabs, floating groups, drag-rearrange, saved layouts) are out of scope. react-resizable-panels gives exactly the resizable/nestable splits needed, at ~10KB zero-dep — matching the client's minimal footprint (~6 runtime deps) — and ships no opinionated CSS to reconcile with the SCSS-modules + CSS-token system (ADR-0006). Pane *content* (toolbar + `TerminalView`) stays decoupled from the splitter, so a later swap to dockview is clean if IDE-style docking is ever genuinely wanted. The umbrella "don't hand-roll docking" constraint is satisfied either way.
