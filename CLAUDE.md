...
- `SessionsScreen.jsx` — thin composition over `useSessions`: cards, filter, and the new-session dialog (which stays open until a create actually succeeds, so a failure surfaces instead of vanishing into an unhandled rejection). The dialog's claude model/effort chips splice flags via `core/claudeFlags.ts`; last-used values persist to `localStorage` on a successful claude spawn and prefill the next claude command (server-side defaults store deferred to spawn-templates phase 2).
