...
- `src/ws.js` — WebSocket hub; each connection is origin-gated (`src/origin.js`) then token-gated before it `attach`es to one line by ID from the URL (`/sessions/:id`). Scrollback replays down the data pipe on connect. Inbound `input` / `resize`; outbound `data` / `exit` (the exit frame carries the code parsed from the board's farewell sentinel). An exited session (tombstone) refuses attach with 1008 — its data pipe is gone. **The `message` and `close` listeners must be registered in the connection handler's first synchronous block, before the `sessions.get`/`sessions.attach` awaits** — a client sends `mode` + its fitted `resize` the moment it sees the 101, and `ws` emits `message` with no listener attached, dropping the frame with no queue or redelivery. Registered late, the PTY stayed at its pre-attach size and every TUI drew for the wrong grid until the operator resized the window by hand; pre-attach frames are buffered (capped — the queue opens before the credential gate) and drained in arrival order. The residual, that the replay is still reconstructed at the pre-attach width, is `_docs/issues/2026-07-30-attach-replay-joiner-width.md`.
...
- `client/src/core/` — the extracted client core: every piece of debugged, non-obvious client logic, in **TypeScript** (core only — screens stay JSX, the server stays CommonJS-no-build by design; `npm run typecheck --workspace=client` checks `src/core` and nothing else). Imports carry explicit `.ts`/`.tsx` extensions so Node's built-in type stripping can run the core's `.test.ts` files directly under `node --test`.
  - `types.ts` — the contracts at the seam: the session DTO (mirrors `server/src/sessions.js` `toDto()`), the WS frame vocabulary, and `TerminalView`'s mode names.
  - `terminalMode.ts` — what a view may do to its line, as two **independent** capabilities: `input` (types into the PTY) and `sizing` (owns its dimensions, so it fits itself, sends resize, and joins the board's smallest-pane clamp). `interactive` = both (mobile terminal, desktop detail pane), `follow` = input only (grid pane, focused), `spectator` = neither (grid pane, watching). **No grid mode owns sizing** — they were fused until 2026-07-30, and the focused pane clamped the shared line to its cell (a 1326x457 cell → a 172x20 PTY), which then *stuck*, because the board frees a pane's clamp entry on control-socket close but keeps the last applied size. Sizing belongs to a view rendering the line at full size; see the ADR-0005 amendment. The `mode` WS frame carries `{input, sizing}` (a pre-split `{spectator}` frame is still honored so a stale tab doesn't lose input); `ws.js` gates each inbound frame on its own capability and opens the board control socket iff `sizing`.
...
**Design system** (`_docs/design-system/` — templates; `client/src/shared/` — the app's copy)

> **`_docs/design-system/` is inspiration only — never import from it in app code.** It's a reference/template kit; the app's real components live in `client/src/shared/`. If you need one there that isn't yet vendored, copy it in and convert it to SCSS Modules like the others — don't wire `client/src` back to `_docs`.

- The app owns its shared UI primitives under `client/src/shared/` (the `@shared` Vite alias points there), as SCSS Modules like the rest of `client/src`. The 9 the app actually uses — `Button`, `Input`, `IconButton`, `StatusDot`, `Card`, `Badge`, `OverflowMenu`, `Kbd`, `Toast` — were vendored out of `_docs/design-system/components/core/` and their runtime-injected `<style>` singletons converted to colocated `.module.scss` (see ADR-0006 amendment). `client/src` no longer reaches into `_docs`.
- `_docs/design-system/` stays as the **inspiration-template kit only** — the originals keep their injected-`<style>` form because `_ds_bundle.js` + the standalone preview pages (`core.card.html`, `ui_kits/…`) load them with no bundler. It is not consumed by the app; changes there do not reach `client/src`.
- **App-owned client UI uses SCSS Modules** (`.module.scss`, colocated, scoped) — `shared/`, `screens/`, `chrome/`, `desktop/`, `App`, and `TerminalView`. Zero static inline `style={{}}` in `client/src` (only genuinely dynamic per-render values stay inline — e.g. `Button`'s `fullWidth` width). `sass` is a client devDep; Vite compiles `.module.scss` (modern API, see `vite.config.js`). Typechecked core (`core/*.tsx`) needs the ambient `*.module.scss` declaration in `core/css.d.ts`. See ADR-0006.
...
|---|---|---|---|
| Attach replay is reconstructed at the PTY's **pre-attach** width — a joiner at a different size gets history laid out for the old grid (app-drawn boxes/rules only; live screen self-corrects on the post-attach SIGWINCH). Residual of the 2026-07-30 dropped-resize-frame fix; needs a board restart to deploy | P3 | M | `_docs/issues/2026-07-30-attach-replay-joiner-width.md` |
...
| libghostty-vt adoption: swap both VT ends (server `@xterm/headless` emulators + client xterm.js → ghostty-web) in one move; parked on upstream API stability, Windows prebuilds, and a VT-sequence serialize API in the Node binding | P3 | L | `_docs/issues/2026-07-23-libghostty-vt-adoption.md` |
