...
| Desktop shell v1: shell split (viewport + manual override) + sidebar master–detail, Ctrl+1..9, local notifications — client-only; umbrella architecture in `_docs/issues/closed/2026-07-02-desktop-workspace-shell.md` (sliced 2026-07-07) | P1 | M | `_docs/issues/2026-07-07-desktop-shell-v1-master-detail.md` |
...
| Inline styles → CSS Modules: no `@media`/`:hover`/`:focus-visible` at the component layer today, hand-rolled per-component in JS (`useMediaQuery`, mouse-handler hover hacks in 4 design-system core components) — reverses a stated design-system convention, needs an ADR before migrating | P3 | M | `_docs/issues/2026-07-09-inline-styles-to-stylesheets.md` |
| Directory browser on the new-session dialog's Working Directory field — server-side `GET /api/fs/browse` (the filesystem is the board's, not the browsing device's) + a breadcrumb picker; complements spawn templates for a not-yet-templated path | P3 | M | `_docs/issues/2026-07-09-session-dialog-directory-browser.md` |
